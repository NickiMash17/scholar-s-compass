...
import {
  ...
  Star, ChevronRight, Globe, BookOpen, Award, Layers,
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* ─── HERO ─── */}
        <section className="container mx-auto px-4 pt-8 sm:pt-16 pb-16 sm:pb-24 section-divider">
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              ...
              <div className="text-center lg:text-left">
                ...
                >
                  {['Project-Based', 'Interview-Ready', 'Industry-Aligned'].map((tag) => (
                    <span key={tag} className="flex items-center gap-1 text-xs sm:text-sm font-medium text-primary/80">
                ...
                >
                  ...
                  <HeroButton variant="ghost" size="lg" onClick={() => navigate('/auth')} className="magnetic-hover">
                    Access Terminal
                  </HeroButton>
                </motion.div>
              ...
              {/* Right – code terminal (replaces static image on desktop) */}
          ...
          {/* Features row */}
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 max-w-5xl mx-auto mt-16 sm:mt-24" staggerDelay={0.12}>
            {features.map((feature) => (
              <StaggerItem key={feature.title}>
                <div className="group relative p-5 sm:p-6 rounded-2xl bg-card/60 border border-border/50 hover:border-primary/30 transition-all duration-500 hover:shadow-glow magnetic-hover frosted-glass">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-4">
                      <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
            ...
            <div className="flex items-center justify-center gap-8 sm:gap-12 flex-wrap">
              {trustLogos.map((name, i) => (
                <motion.span
                  ...
                  className="text-sm sm:text-base font-semibold text-muted-foreground/40 tracking-wide select-none hover:text-primary/50 transition-colors duration-300"
                >
                  {name}
                </motion.span>
              ))}
        ...
        {/* ─── HOW IT WORKS ─── */}
        <section className="container mx-auto px-4 py-20 sm:py-32 section-divider">
          ...
          <StaggerContainer className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8" staggerDelay={0.1}>
            {howItWorks.map((item) => (
              <StaggerItem key={item.step}>
                <div className="group relative p-6 sm:p-8 rounded-2xl bg-card/40 border border-border/40 hover:border-primary/30 transition-all duration-500 hover:shadow-glow magnetic-hover h-full">
                  <div className="absolute top-6 right-6 sm:top-8 sm:right-8">
                    <span className="font-mono text-3xl sm:text-4xl font-black text-primary/10 group-hover:text-primary/25 transition-colors duration-500">
                      {item.step}
                    </span>
                  </div>
                  <div className="w-12 h-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center mb-5 group-hover:bg-primary/15 group-hover:scale-110 group-hover:shadow-glow transition-all duration-300">
        ...
        {/* ─── TOPICS ─── */}
        <section id="topics" className="container mx-auto px-4 py-16 sm:py-24 section-divider">
          ...
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 max-w-7xl mx-auto" staggerDelay={0.06}>
            ...
            <StaggerItem>
              <motion.button
                ...
                className="group relative w-full overflow-hidden rounded-2xl p-6 text-left transition-all duration-500 ease-out border border-dashed border-primary/20 hover:border-primary/50 hover:shadow-glow focus-ring bg-card/30 backdrop-blur-sm magnetic-hover"
              >
                <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-center">
                  ...
                  <h3 className="text-xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors">Custom Protocol</h3>
                  <p className="text-sm text-muted-foreground">Define your own study vector for any subject</p>
                </div>
              </motion.button>
            </StaggerItem>
          </StaggerContainer>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto" staggerDelay={0.12}>
            {testimonials.map((t) => (
              <StaggerItem key={t.name}>
                <div className="relative p-6 sm:p-8 rounded-2xl frosted-glass hover:border-primary/20 transition-all duration-500 magnetic-hover h-full">
                  <div className="flex items-center gap-1 mb-4">
                    {[...Array(5)].map((_, s) => (
                      <Star key={s} className="w-3.5 h-3.5 text-primary fill-primary" />
                    ))}
                  </div>
                  <p className="text-sm sm:text-base text-foreground/90 leading-relaxed mb-6 italic">"{t.quote}"</p>
        ...
        {/* ─── LEARNING ROADMAP ─── */}
        <section className="container mx-auto px-4 py-16 sm:py-24 section-divider">
          <SectionReveal className="text-center mb-10 sm:mb-14">
        ...
        <section className="container mx-auto px-4 py-20 sm:py-32">
          <SectionReveal>
            <div className="relative max-w-4xl mx-auto text-center p-10 sm:p-16 rounded-3xl border border-primary/20 bg-card/30 backdrop-blur-sm overflow-hidden shimmer-border">
          ...
          <div className="container mx-auto px-4 py-12 sm:py-16">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 sm:gap-12 mb-10 sm:mb-14">
              ...
              {[
                ...
                <div key={col.title}>
                  <h4 className="font-mono text-[10px] uppercase tracking-[0.15em] text-primary/70 mb-4">{col.title}</h4>
